  ...
  case ${1} {
    clang)
      if (( ${+commands[clang-format-13]} )) {
        ...
        if ! is-at-least 13.0.0 ${formatter_version[-1]}; then
          log_error "clang-format is not version 13.0 or above (found ${formatter_version[-1]}."
          exit 2
        fi

        if ! is-at-least ${formatter_version[-1]} 13.0.1; then
          log_error "clang-format is more recent than version 13.0 (found ${formatter_version[-1]})."
          exit 2
        fi
      } else {
